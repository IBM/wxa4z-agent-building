...
2. Once done, edit the `.env.agent.zrag-agent` file using `nano` or `vi` text editor. For example:
    ...
    For the **zRAG Agent**, most of the configuration is already done, including the core services. The only required environment variable for the zRAG Agent is the `AGENT_AUTH_TOKEN` variable.
